...
def create_app():
    ...
    # New route matching backend pipelineTrigger.ts
    ...
    def process_claim_legacy(claim_id):
        try:
            app.logger.info(f'Processing claim with id "{claim_id}" (legacy route)')
            process_targeted(claim_id)
...
if __name__ == "__main__":
    ...
    # Run the server - default to 8001 to match backend expectation
    serve(app,
          host="0.0.0.0",
          port=APP_PORT or 8001,
          threads=10,
