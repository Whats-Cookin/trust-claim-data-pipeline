...
def get_composedb_cmd(claim_json):
    if (
        claim_json.get("effectiveDate")
        and type(claim_json["effectiveDate"]) == datetime.datetime
    ):
...
def is_useful(raw_claim):
    return bool(
        raw_claim.get("claim")
        and raw_claim.get("subject")
        and (raw_claim.get("statement") or raw_claim.get("object"))
        and raw_claim.get("effectiveDate")
    )
...
def publish_unpublished():
    for raw_claim in unpublished_claims_generator():
        if is_useful(raw_claim):
            ...
            print(
                "Skipping claim about "
                + raw_claim.get("subject")
                + " bc of missing fields"
            )
...
def make_compose_json(raw_claim):
    ...
    compose_json["subjectID"] = raw_claim[
        "subject"
    ]  # should be already normalized but don't change it
    ...
    if raw_claim.get("source"):
        ...
        compose_json["source"] = {
            "howKnown": how_known,
            "sourceID": raw_claim["source"],
        }

    if raw_claim.get("score") or raw_claim.get("stars"):
        compose_json["rating"] = {
            k: raw_claim[k] for k in ("aspect", "stars", "score") if raw_claim.get(k)
        }
