...
from lib.cleaners import normalize_uri, construct_uri, make_subject_uri
from lib.db import (get_conn, get_claim, unprocessed_claims_generator, 
                   unpublished_claims_generator, execute_sql_query, 
                   update_claim_address, insert_data, insert_node, 
                   insert_edge, get_node_by_uri, get_edge_by_endpoints,
                   del_claim)
from lib.infer import infer_details, open_display, close_display
...
def test_construct_uri():
    ...
    assert construct_uri('test', 'custom') == 'https://linkedtrust.us/issuer/custom/labels/test'
...
@pytest.mark.skipif(not SELENIUM_AVAILABLE or not check_xvfb_installed() or not check_chrome_installed(),
