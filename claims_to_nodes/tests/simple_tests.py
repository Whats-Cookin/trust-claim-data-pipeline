...
class TestPipe(unittest.TestCase):
    ...
    def test_get_or_create_node(
        ...
        mock_insert_node,
        mock_infer_details,
        mock_get_node_by_uri,
        mock_normalize_uri,
    ):
        raw_claim = {"subject": "subject_uri", "issuerId": 1}
        ...
        mock_normalize_uri.assert_called_once_with(
            raw_claim["subject"], raw_claim.get("issuerId", 1)
        )
        mock_get_node_by_uri.assert_called_once_with("normalized_uri")
        mock_infer_details.assert_called_once_with(
            "normalized_uri", save_thumbnail=True
        )
    ...
    def test_get_or_create_edge(self, mock_insert_edge, mock_get_edge_by_endpoints):
        ...
        mock_get_edge_by_endpoints.assert_called_once_with(
            start_node["id"], end_node["id"], claim_id
        )
    ...
    @patch("claims_to_nodes.pipe.process_targeted")
    def test_process_unprocessed(
        self, mock_process_targeted, mock_unprocessed_claims_generator
    ):
        process_unprocessed()
        mock_unprocessed_claims_generator.assert_called_once()
        mock_process_targeted.assert_called_once_with({"id": 1})
    ...
    def test_process_claim_with_object(
        self, mock_get_or_create_edge, mock_get_or_create_node
    ):
        raw_claim = {
            "subject": "subject_uri",
            "object": "object_uri",
        ...
        mock_get_or_create_edge.assert_called_once_with(
            {"id": 1}, {"id": 2}, "relationship", 1
        )

    @patch(
        "claims_to_nodes.get_or_create_node", side_effect=[{"id": 1}, None, {"id": 3}]
    )
    @patch("claims_to_nodes.get_or_create_edge")
    def test_process_claim_without_object(
        self, mock_get_or_create_edge, mock_get_or_create_node
    ):
        raw_claim = {
            "subject": "subject_uri",
