...
from lib.db import (get_claim, get_edge_by_endpoints, get_node_by_uri,
                    insert_edge, insert_node, unprocessed_claims_generator)
...
def make_description(raw_claim):
    descrip = ''
    if raw_claim['score']:
        descrip += "{} score: {:.2%}".format(raw_claim.get('aspect', ''),
                                             raw_claim['score'])
...
def process_claim(raw_claim):
    ...
    if object_node:
        get_or_create_edge(subject_node, object_node,
                           raw_claim['claim'], raw_claim['id'])
    ...
    # in this case we make a claim node and also attach the source
        ...
        get_or_create_edge(subject_node, claim_node, raw_claim['claim'],
                           raw_claim['id'])

        # create the edge from the claim node to the source node
        if source_node:
            get_or_create_edge(claim_node, source_node, 'source',
                               raw_claim['id'])
