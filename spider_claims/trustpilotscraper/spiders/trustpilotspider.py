...
class TrustpilotSpiderSpider(Spider):
    ...
    def parse_company_page(self, response):
        address_list = response.css(
            "ul.styles_contactInfoElements__YqQAJ li.styles_contactInfoElement__SxlS3"
        )

        company_item = CompanyItem()

        company_item["url"] = response.url
        company_item["company_name"] = response.css(
            "#business-unit-title h1.title_title__i9V__ span.title_displayName__TtDDM::text"
        ).get()
        try:
            company_item["company_mail_address"] = (
                address_list[0].css(".link_link__IZzHN::text").get()
            )
        except:
            company_item["company_mail_address"] = "NIL"
        try:
            company_item["company_geo_location"] = (
                address_list[1]
                .css(".styles_contactInfoAddressList__RxiJI li::text")
                .get()
            )
        except:
            company_item["company_geo_location"] = "NIL"
        company_item["company_image_source"] = response.css(
            "div.styles_summary__gEFdQ div.profile-image_imageWrapper__kDdWe a picture img::attr(src)"
        ).get()
        company_item["company_category"] = response.css(
            "li.breadcrumb_breadcrumb__lJO__ a::text"
        ).extract()
        company_item["company_trustpilot_rating"] = response.css(
            "#business-unit-title div.styles_container__OaEK8 div.styles_rating__NPyeH p::text"
        ).get()
        company_item["verified"] = response.css(
            "#business-unit-title button.styles_verificationLabel__kukuk div.styles_verificationIcon___X7KO::text"
        ).get()
        company_item["number_of_reviews"] = response.css(
            "#business-unit-title span.styles_clickable__zQWyh span.styles_text__W4hWi::text"
        ).get()
