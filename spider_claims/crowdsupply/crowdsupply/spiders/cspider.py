...
class CSpider(Spider):
    ...
    def parse_projectpage(self, response):
        ...
        try:
            project_name = response.xpath('/html/body/section[1]/div/h1/a/text()').extract_first()
            if project_name:
                project_url = response.url
                project_description = response.xpath('/html/body/section[1]/div/h3/text()').extract_first()
                raised_amount = response.xpath('/html/body/section[1]/div/div/div[2]/div/div[1]/div/p[1]/span/text()').extract_first()
                goal_amount = response.xpath('/html/body/section[1]/div/div/div[2]/div/div[1]/div/p[2]/span/text()').extract_first()
                date_funded_days_to_be_funded = response.xpath('/html/body/section[1]/div/div/div[2]/div/div[1]/div/div[2]/div[2]/a/span[1]/text()').extract_first()
                number_of_backers = response.xpath('/html/body/section[1]/div/div/div[2]/div/div[1]/div/div[2]/div[3]/a/span[1]/text()').extract_first()
                ...
                request = scrapy.Request(url=backers_url, callback=self.parse_backerspage, errback=self.handle_error)
                ...
                if number_of_backers:
                    try:
                        ...
                        for page in range(2, number_of_backers_pages + 1):
                            ...
                            request = scrapy.Request(url=paginated_backers_url, callback=self.parse_backerspage, errback=self.handle_error)
                            request.meta['project_data'] = project_data
                            yield request
                    except ValueError:
                        self.logger.error(f"Failed to convert number of backers to integer: {number_of_backers}")
            else:
    ...
    def parse_backerspage(self, response):
        ...
        try:
            ...
            self.logger.error(f"KeyError: {e} - Missing 'project_data' in meta for URL: {response.url}")
        except Exception as e:
