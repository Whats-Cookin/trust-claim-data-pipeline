...
def scrape_data(url):
    ...
    if progress_meter:
        ...
        goal_amount = re.search(r'(\d[\d.,]*)', goal_amount_element.text.strip()).group() if goal_amount_element and re.search(r'(\d[\d.,]*)', goal_amount_element.text.strip()) else "Goal amount not found"
    else:
